class Cart {
  ...
  async renderCart() {
    ...
    this.cartContainer.querySelector('.cart-product-list-container').innerHTML =
      cartDomSting;
    this.cartContainer
  ...
  async updateBadge() {
    ...
    document.querySelector(
      '#cart-badge'
    ).innerText = `${count} шт. -  ${cost.toFixed(2)} грн`;
    if (count === 0) {
  ...
  async order(ev) {
    if ((await this.cartLengthAndCost()).count === 0) {
      window.showAlert('Виберіть товар для замовлення!', false);
      return;
    }
    ...
    if (form.checkValidity()) {
      ...
      fetch('order', {
        ...
        body: JSON.stringify({
          ...
          clientEmail: document.querySelector('#client-tel').value,
          cart: this.cart,
        }),
      })
        .then((response) => {
      ...
      window.showAlert('Заповність форму для відправки коректно', false);
    }
  }
}
